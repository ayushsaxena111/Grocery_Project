...
const OrderTracking = () => {
    ...
    useEffect(()=>{
        api.get(`/orders/4{id}`).then((res)=>setOrder(res.data.order)).catch(()=>navigate("/orders")).finally(()=>setLoading(false))
    },[id,navigate])
        ...
        <div className="min-h-screen mb-20 bg-app-cream">
            <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="flex items-center justify-between mb-8 ">
                    ...
                    <span className={`px-4 py-1.5 text-sm font-semibold rounded-full ${order!.status === "Delivered" ? "bg-green-100 text-green-700" : order!.status === "Cancelled" ? "bg-red=100 text-red-800" : "bg-app-orange/10 text-app-orange"}`}>
                        {order!.status}
                    </span>
                </div>
